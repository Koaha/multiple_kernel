...
class NystromApproximation:
    ...
    def __init__(self, X, kernel, sample_size, sampling_method='random', regularization=1e-5, support_indices=None):
        self.X = X
        self.kernel = kernel
    ...
    def _select_samples(self):
        ...
        if self.sampling_method == 'random':
            ...
            selected_indices = np.argmin(np.linalg.norm(self.X[:, None] - centers, axis=2), axis=1)
            return np.unique(selected_indices)[:self.sample_size]
        
        elif self.sampling_method == 'support_vectors' and self.support_indices is not None:
            ...
            raise ValueError("Invalid sampling method or missing required support_indices.")
